<?php namespace VanicaCummings\Helpers;
...
class AssetPath {
  ...
  public static function get($path, $manifest_file = null) {
    if (!$manifest_file) {
      $manifest_file = JCDWP_THEME_ASSET_DIR . '/' . JCDWP_THEME_ASSET_REV;
    }
    if (!file_exists($manifest_file)) {
      return JCDWP_THEME_ASSET_URI . '/' . $path;
    } else {
      ...
      foreach ($manifest as $key => $val) {
        if ($filename === $key) {
          ...
          return JCDWP_THEME_ASSET_URI . '/' . $file_path;
        }
      }
    }
  }
